...
namespace Microsoft.Samples.Kinect.BodyBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        /// </summary>
        private DateTime serialPortLastWriteTime1 = DateTime.Now;
        private DateTime serialPortLastWriteTime2 = DateTime.Now;
        private DateTime serialPortLastWriteTime3 = DateTime.Now;
        ...
        /// </summary>
        private TimeSpan serialPortMaxWriteInterval = TimeSpan.FromMilliseconds(200);
        private TimeSpan serialPortMaxWriteInterval2 = TimeSpan.FromMilliseconds(2000);
        private TimeSpan serialPortMaxWriteInterval3 = TimeSpan.FromMilliseconds(2000);
        ...
        {
            string line = arduinoSerialPort.ReadLine();
                ...
                using (DrawingContext dc = this.drawingGroup.Open())
                {
                    ...
                    foreach (Body body in this.bodies)
                    {
                        ...
                        if (body.IsTracked)
                        {
                            ...
                            // Lets do something with head, hand, and movement 
                            Joint rightHand = body.Joints[JointType.HandTipRight];

                            TrackingState rightHandState = rightHand.TrackingState;

                            //for state 2
                            if (rightHandState == TrackingState.Tracked)
                            {
                                ...
                                {
                                    byte handPosX = (byte)Math.Round(x * 9);
                                    byte handPosY = (byte)Math.Round(y * 9);
                            ...
                            if (headState == TrackingState.Tracked)
                            {
                                ...
                                Console.WriteLine("headJoint.Position: ({0}, {1}, {2})",
                                     x, y, z);
                                ...
                                if (DateTime.Now - serialPortLastWriteTime3 > serialPortMaxWriteInterval3)
                                {
                                    ...
                                    if ((z - prevZ) * 255 < -10)
                                    {
                                        ...
                                        // Timestamp for last write time
                                        serialPortLastWriteTime3 = DateTime.Now;
                            ...
                            //    DepthSpacePoint depthPoint = this.kinectSensor.CoordinateMapper.MapCameraPointToDepthSpace(head.Position);

                            //    float x = colorPoint.X;
                            //    float y = colorPoint.Y;
                            //    float z = head.Position.Z;

                            //    // Passing the values via serial port to Arduino 

                            //    //Console.WriteLine("headJoint.Position: ({0}, {1}, {2})",
                            //    // x, y, z);

                            //    if (DateTime.Now - serialPortLastWriteTime > serialPortMaxWriteInterval)
                            //    {
                            //        byte headPosX = (byte)Math.Round(x * 9);
                            //        byte headPosY = (byte)Math.Round(y * 9);

                            //        byte[] byteArray = new byte[1];
                            //        byteArray[0] = headPosX;
                            //        arduinoSerialPort.Write(byteArray, 0, 1);

                            //        // Just some debugging
                            //        Trace.WriteLine(String.Format("Sent to Arduino: {0}", headPosX));

                            //        // Timestamp for last write time
                            //        serialPortLastWriteTime = DateTime.Now;
                    ...
                    if (currentBodyCount != previousBodyCount)
                    {
                        ...
                        if (currentBodyCount > previousBodyCount)
                        {
                            // writing that state 1 needs to be triggered
                            byte message = (byte)1;
                            ...
                            Trace.WriteLine(String.Format("Sent to Arduino: S1"));
                        }
